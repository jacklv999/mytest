...
for i in tmp_str_3:
    if str(i)[-5:-1]=='html':
        ...
        fp = open('D:\\PythonCode\\Pages\\mytest\\mytest\\README.md','r+',encoding='UTF-8')
        s = fp.read()                   #将指定文件读入内存
        fp.close()
        a = s.split('\n')
        ...
        s = '\n'.join(a)                #用'\n'连接各个元素
        #print(s)
        fp = open('D:\\PythonCode\\Pages\\mytest\\mytest\\README.md','w',encoding='UTF-8')
        fp.write(s)
        fp.close()
...
for root,dirs,files in os.walk("D:\PythonCode\Pages\mytest\mytest\读书笔记"):
    with open('D:\\PythonCode\\Pages\\mytest\\mytest\\读书笔记\\1.txt','a') as f:
        for file in files:
