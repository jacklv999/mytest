...
- 5.以可行广义最小二乘法求解:
  ...
  - 3.附加GMM方法: 
    ...
    - 3.定义: $ \hat{\overline{u}}_i = \rho \sum_{i = 1}^nw_{ij}\hat{u}_j + \epsilon_i $ , $ \hat{\overline{\overline{u}}}_i = \sum_{i = 1}^nw_{ij}\hat{\overline{u}}_j  $  
      ...
      $\begin{cases} \frac{1}{n}\sum_{i = 1}^n(\hat{u_i} - \rho \hat{\overline{u}}_i)^2 = \sigma_{\epsilon}^2 \\  \\  \frac{1}{n}\sum_{i=1}^n (\hat{\overline{u}_i}- \rho \hat{\overline{\overline{u}}}_i) = \sigma_{\epsilon}^2tr\frac{W^TW}{n}\\ \\ \frac{1}{n}\sum_{i=1}^n(\hat{u}_i-\rho \hat{\overline{u}}_i)(\hat{\overline{u}}_i-\hat{\overline{\overline{u}}}_i)  = 0   \end{cases}  $
...
- 2.模型估计如下: 
  ...
  - 2.两阶段最小二乘法:
    ...
    - 2.自变量M对工具变量H回归:  $M = H\gamma + \eta$ , 其中 η 为误差项, 得出估计值为: $\hat{\gamma} = (H^TH)^{-1}H^TM$ 
    - 3.计算M的估计值:  $ \hat{M} = H\hat{\gamma} $ , 并计算方程:  $ y = \hat{M}\theta + u $, 得: $ \hat{\theta}_{2SLS} = (\hat{M}^T\hat{M})^{-1}\hat{M}^{-1}y $ 
...
- 4.使用广义空间两阶段最小二乘法:
  - 1.利用工具变量 Z 和 WZ 以2SLS方式估计方程:  $ y = Z\beta +\lambda Wy + u $ 得到 $\tilde{\beta}$ 和 $ \tilde{\lambda} $ 
  - 2.以方程: $ \hat{u} = y-Z\tilde{\beta} - \tilde{\lambda} WY $, 并定义 $ \hat{\overline{u}} = W\hat{u} $, $ \hat{\overline{\overline{u}}} = W^2 \hat{u} $ 
  - 3.以
