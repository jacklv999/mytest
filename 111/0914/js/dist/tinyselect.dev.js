...
(function ($) {
  ...
  var TinySelect = {
    ...
    createSelect: function createSelect($el) {
      // Create container for select, search and options
      this.state.container = $("<div></div>").addClass("tinyselect").css({
        width: $el.css("width")
      }); // Create the select element

      this.state.selectBox = $("<div></div>").addClass("selectbox").on("click", {
        self: this
      }, this.onSelectBoxClicked);
      this.state.container.append(this.state.selectBox); // Create container to hold search and results

      this.state.dropdown = $("<div></div>").addClass("dropdown").hide();
      this.state.container.append(this.state.dropdown); // Add search as first element

      if (this.config.showSearch) this.createSearch(this.state.dropdown); // Create ul to hold items		

      this.state.itemContainer = $("<ul></ul>").addClass("itemcontainer");
      this.state.dropdown.append(this.state.itemContainer); //

      this.createItems(); // Hide original select element and add new component to below

      $el.hide().after(this.state.container);
      this.state.$el = $el; // Hide select content when clicked elsewhere in the document

      $(document).on("click", {
        self: this
      }, this.onDocumentClicked);
    },
    createItems: function createItems(selected) {
      var l1, opt; // Remove all 
    ...
    createSearch: function createSearch($el) {
    ...
    readSelect: function readSelect($el) {
      var self = this;
    ...
    setAjaxIndicator: function setAjaxIndicator(failure) {
     ...
     * Event handlers
     * ******************************************************************* */
    onDocumentClicked: function onDocumentClicked(e) {
    ...
    onSelectBoxClicked: function onSelectBoxClicked(e) {
      var self = e.data.self; // Do nothing, if currently animating

      if (self.state.dropdown.is(":animated")) return; // Close selectBox
    ...
    onAjaxLoadSuccess: function onAjaxLoadSuccess(self, data) {
    ...
    onAjaxLoadError: function onAjaxLoadError(self, data) {
      self.setAjaxIndicator(true);
    },
    onSelectLiClicked: function onSelectLiClicked(e) {
      var self = e.data.self,
