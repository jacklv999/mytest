...
(function ($) {
  ...
  var extensionMethods = {
    pips: function pips(settings) {
      ...
      slider.options.pipStep = options.step; // get rid of all pips that might already exist.

      slider.element.addClass("ui-slider-pips").find(".ui-slider-pip").remove(); // small object with functions for marking pips as selected.

      var selectPip = {
        single: function single(value) {
          var $pips = this.resetClasses();
          $pips.filter(".ui-slider-pip-" + value).addClass("ui-slider-pip-selected");
        },
        range: function range(values) {
          var $pips = this.resetClasses();
          $pips.filter(".ui-slider-pip-" + values[0]).addClass("ui-slider-pip-selected-first");
          $pips.filter(".ui-slider-pip-" + values[1]).addClass("ui-slider-pip-selected-second");
        },
        resetClasses: function resetClasses() {
          var $pips = slider.element.find(".ui-slider-pip").removeClass("ui-slider-pip-selected ui-slider-pip-selected-first ui-slider-pip-selected-second");
          return $pips;
        }
      ...
      function labelClick(label) {
        ...
        if (true === slider.options.range) {
          ...
          var finalVals; // If the handles are together when we click a label...

          if (sliderVals[0] === sliderVals[1]) {
            // ...and the label we clicked on is less,
            // then move first handle to the label...
            if (val < sliderVals[0]) {
              finalVals = [val, sliderVals[1]]; // ...otherwise move the second handle to the label
            } else {
            ...
            finalVals = [val, val]; // ...or if the second handle is closest to our label, bring second
            // handle to the label...
          } else if (Math.abs(sliderVals[0] - val) < Math.abs(sliderVals[1] - val)) {
            finalVals = [val, sliderVals[1]]; // ...or if the first handle is closest to our label, bring that handle.
          } else {
            finalVals = [sliderVals[0], val];
          }
      ...
      function createPip(which) {
        ...
        var labelValue = slider.options.min + slider.options.step * number; // classLabel replaces any decimals with hyphens

        var classLabel = labelValue.toString().replace(".", "-"); // We need to set the human-readable label to either the
        ...
        if ("first" === which) {
          ...
          classes += false === options.first ? " ui-slider-pip-hide" : ""; // Last Pip on the Slider
        } else if ("last" === which) {
          percent = "100%";
          classes += " ui-slider-pip-last";
          classes += "label" === options.last ? " ui-slider-pip-label" : "";
          classes += false === options.last ? " ui-slider-pip-hide" : ""; // All other Pips
        } else {
      ...
      slider.options.pipStep = Math.round(slider.options.pipStep); // create our first pip

      collection += createPip("first"); // for every stop in the slider; we create a pip.
      ...
      collection += createPip("last"); // add special classes to the pips that were set initially.

      var oldClass, newClass;

      if (slider.options.values) {
        oldClass = ["ui-slider-pip-" + slider.options.values[0], "ui-slider-pip-" + slider.options.values[1]];
        newClass = ["ui-slider-pip-" + slider.options.values[0] + " ui-slider-pip-selected-initial-first", "ui-slider-pip-" + slider.options.values[1] + " ui-slider-pip-selected-initial-second"];
        collection = collection.replace(oldClass[0], newClass[0]).replace(oldClass[1], newClass[1]);
      } else {
        oldClass = "ui-slider-pip-" + slider.options.value;
        newClass = "ui-slider-pip-" + slider.options.value + " ui-slider-pip-selected-initial";
        collection = collection.replace(oldClass, newClass);
    ...
    "float": function float(settings) {
      ...
      function getPipLabels(val, val2) {
        ...
        var vals = [],
            ...
            stepVal2 = val2 / slider.options.step; // now we just get the values we need to return
      ...
      if (options.handle) {
        ...
        if (slider.options.values) {
          ...
          $tip = [$("<span class=\"ui-slider-tip\">" + options.formatLabel(vals[0]) + "</span>"), $("<span class=\"ui-slider-tip\">" + options.formatLabel(vals[1]) + "</span>")]; // else if its just a normal slider
        } else {
          vals = getPipLabels(slider.options.value); // create a tip element
      ...
      if (options.pips) {
        ...
        slider.element.find(".ui-slider-label").each(function (k, v) {
          var $this = $(v),
              ...
              $tip;
          label = getPipLabels(val)[0]; // create a tip element
