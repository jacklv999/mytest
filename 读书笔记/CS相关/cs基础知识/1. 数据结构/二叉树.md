...
def levelOrder(self, root: TreeNode) -> List[List[int]]:
    ...
    layer, ans = [root], []
    while True:
        ans.append([X.val for X in layer])
        if not (layer := getNextLayer(layer)):
            break
...
def levelOrder(self, root):
    ...
    stack.append([root,0])
    while stack:
        cur_node,cur_dep = stack.popleft()
        d[cur_dep].append(cur_node.val)
        if  cur_node.left:
            stack.append(
                [cur_node.
                 left,cur_dep+1
...
```python
def PthS(self, root, targetSum):
    if not root:
        return False
    if targetSum == root.val and not root.left and not root.right:
        return True

    next_tgt=targetSum-root.val

    if root.left:
        if self.PthS(root.left,next_tgt):
            return True 
    if root.right:
        if self.PthS(root.right,next_tgt):
            return True 
    return False
```
